...
class iciba:
    ...
    def send_msg(self, openid, template_id, day_profit, userid):
        msg = {
            ...
            'url': "http://www.btcat.cn/my.html?usrid=" + userid,
            'data': {
                'content': {
    ...
    def send_everyday_words(self, openids):
        ...
        result = self.send_msg(openid_chen, self.template_id, day_profit_chen, openid_w)
...
if __name__ == '__main__':
    file_path =  '/var/www/html/wx/'
    day_profit_gxj = round(159000 * 0.31 / 365 + random.random() * 10, 2)
    day_profit_w = round(500000 * 0.31 / 365 + random.random() * 8, 2)
    #新增---test---
    ...
    openid_liubai = 'oeuIhv4VTgg2YMd8qNlPYM9JEGgI' #留白
    
    # 执行
    icb = iciba(wechat_config)
    content = icb.get_local_json(openid_w)
    icb.add_profit_json(content, openid_w, day_profit_w)
    content = icb.get_local_json(openid_gxj)
    icb.add_profit_json(content, openid_gxj, day_profit_gxj)
    icb.run(openids)
